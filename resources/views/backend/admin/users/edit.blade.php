        ...
        <div class="w-full mx-20 my-20">
            ...
            <form action="{{ route('backend.user.edit.process', $user->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" value="{{ $user->service_sekolah->id }}" id="sekolah" name="sekolah"
                        class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 text-sm"
            ...
            <div class="mb-4 flex items-center space-x-2">
                @if ($user->roles)
                    ...
                    @foreach ($user->roles as $user_role)
                        <form action="{{ route('backend.user.delete.role', [$user->id, $user_role->id]) }}" class="flex"
                            method="post">
                            @csrf
                            @method('DELETE')
            ...
            <div class="max-w-full">
                ...
                <form action="{{ route('backend.user.edit.assign.role', $user->id) }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    <label for="permission" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Select
                        an option</label>
                    ...
                    <button type="submit"
                        class="text-white mt-5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Assign</button>
                </form>
